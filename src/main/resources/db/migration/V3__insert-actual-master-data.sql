...
INSERT INTO tbm_assessment_topic_reference(topic, rating, reference)
    VALUES ( 1, 'ONE', 'Big ball of mud monolith. Dependencies are Ad-hoc, little to no separation between core and third-party application logic. Changes in third-party integration logic usually requires major changes in a majority of systems, including user interfaces.');
INSERT INTO tbm_assessment_topic_reference(topic, rating, reference)
    VALUES ( 1, 'TWO', 'Structured Monolith. Separation between core and third-party components exists. However, changes to third-party components usually results in major changes to service components directly interfacing with these third party components.');
INSERT INTO tbm_assessment_topic_reference(topic, rating, reference)
    VALUES ( 1, 'THREE', 'Service Oriented Microservices. Separation between core and third-party components exists. Logic is cleanly veiled off in third party components. However, there is little clarity of non-functional resilience characteristics.');
INSERT INTO tbm_assessment_topic_reference(topic, rating, reference)
    VALUES ( 1, 'FOUR', 'Event-Driven Microservices and service-oriented microservices are used as applicable. Separation between core and third-party components exists. Logic and non-functional requirements are cleanly veiled off in third party components. However, each team uses its own set of implementations when integrating with third parties.');
INSERT INTO tbm_assessment_topic_reference(topic, rating, reference)
    VALUES ( 1, 'FIVE', 'Serverless Functions, Event-Driven Microservices and service-oriented microservices are used as applicable. Standardized integration templates exist and are actively used. These standardized templates are designed with specific care being taken to isolate third party logic and resilience expectations.');

----------

INSERT INTO tbm_assessment_topic_reference(topic, rating, reference)
VALUES ( 2, 'ONE', 'API gateway is not used as an explicit pattern. No explicit strategy is used for API versioning. API Consumers are notified if backwards incompatible changes are made and are expected. API documentation is largely missing or ignored. API clients are required to consult producers'' source code to look for usage patterns. No explicit strategy is used for API versioning. API Consumers are notified if backwards incompatible changes are made and are expected. API documentation is largely missing or ignored. API clients are required to consult producers'' source code to look for usage patterns.');
INSERT INTO tbm_assessment_topic_reference(topic, rating, reference)
VALUES ( 2, 'TWO', 'The use of API gateway is explicitly discouraged to avoid it becoming a home of excessive business logic. API versioning is used very sparingly. In most cases, changes are made in a backwards compatible manner. When backwards incompatible changes are required to be made, new APIs are created. API documentation is produced at the start of a project, but is not maintained up-to-date. Hence is used sparingly by consuming teams.');
INSERT INTO tbm_assessment_topic_reference(topic, rating, reference)
VALUES ( 2, 'THREE', 'API Gateway is used as a single entry point for all consumers. It acts as a simple proxy for all downstream services and as a switch to enable zero downtime deployments. URL based API versioning is used at all times. Consumers are expected to include a version identifier in the request at all times. API documentation is produced and all efforts are made to keep it up-to-date with with implementation changes. However, documentation does experience drift from implementation because it is maintained separately from the implementation.');
INSERT INTO tbm_assessment_topic_reference(topic, rating, reference)
VALUES ( 2, 'FOUR', 'API Gateway is used as a single entry point for all consumers. It is used to expose different variations of the same API to different consumers and as a switch to enable zero downtime deployments. Header-based API versioning is used at all times. Consumers are expected to include a version identifier in the request at all times. API documentation is produced and kept in sync with implementation at all times. Documentation and implementation are produced from the same source repository.');
INSERT INTO tbm_assessment_topic_reference(topic, rating, reference)
